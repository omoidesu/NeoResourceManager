        ...
        <n-layout-content class="main-content" content-style="height: 100%;">
          <div class="content-shell">
            <router-view/>
              ...
              <div class="mini-audio-player__cover" @click="audioPlayerStore.controls.value.reopen?.()">
                <img
              ...
              <div class="mini-audio-player__meta" @click="audioPlayerStore.controls.value.reopen?.()">
                <div class="mini-audio-player__eyebrow">{{ audioPlayerStore.title.value }}</div>
                <div class="mini-audio-player__title">{{ audioPlayerStore.currentTrack.value?.label }}</div>
            ...
            <div v-if="hasOngoingItems" class="notification-center__section">
              ...
              <div v-if="notificationCenterStore.ongoingItems.value.length" class="notification-center__list">
                <div
                  ...
                  :style="item.kind === 'audio-player' && item.meta?.coverSrc ? { backgroundImage: `linear-gradient(135deg, rgba(10, 14, 18, 0.88), rgba(16, 18, 24, 0.82)), url(${item.meta.coverSrc})` } : undefined"
                  @click="item.onClick?.()"
                >
                  <template v-if="item.kind === 'audio-player'">
                    <div class="notification-center__mini-player">
                      ...
                      <div class="notification-center__mini-player-title">{{ item.meta?.trackLabel || getOngoingPrimaryText(item.content) }}</div>
                      <div class="notification-center__mini-player-subtitle">{{ item.meta?.title || getOngoingSecondaryText(item.content) }}</div>
                      <n-progress
                        type="line"
