...
- 分类详情页完成组件化重构，拆分出批量导入、筛选、详情抽屉、编辑抽屉、阅读/播放、预览资源与能力动作等独立模块，后续新增分类能力和维护成本明显下降。
- 首页 Dashboard 进入可用版本，补齐欢迎区、资源概览、继续使用、接下来看看、首页固定、资源分析、活跃热力图、资源健康、封面墙、回到顶部与亮暗主题适配。
- 批量导入修复切换分类时状态串用问题，避免在上一轮批量导入预览未清理时把资源错误导入到新分类。
- 封面墙支持数据库搜索、分页无限滚动、吸顶筛选、封面悬浮信息和大数据量下的懒渲染优化。
- 数据库补齐 `home_pin`、`if_top`、`search_text` 等迁移与兼容逻辑，资源搜索支持标签、分类、别名、作品 ID、专辑、作者、演员/声优等更多关键词。
