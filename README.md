...
- 优化视频播放链路，补齐直放 / 转封装 / 流式转码决策、失败回传与前后端排障日志。
- 优化归档包管理，新增失效识别、后台 watcher 同步与表格实时刷新。
- 优化网站资源后台回填反馈，补充进度提示与网页截图稳定性等待。
- 修复打包版日志落盘与媒体二进制路径处理，改善发布包问题定位能力。
...
- 资源失效监听与自动重定位能力
- 资源治理工作台，支持问题资源处理、标签/分类治理与归档包管理
- 资源归档队列，支持单资源归档、批量归档、合包归档、还原与删除归档包
- 本地 API 服务，支持查询应用版本与各类型资源数量
- 批量导入游戏目录
- 首页 Dashboard，包含资源概览、继续使用、接下来看看、首页固定、资源分析、活跃热力图与封面墙
- 首页固定与资源置顶能力
- 电影资源管理，支持电影文件识别、分类整理与本地播放/预览
- 电影默认首帧封面生成、播放进度记录与续播完成阈值设置
- 番剧目录管理、分集同步、分集顺序调整与番剧目录批量导入
- 音声目录树浏览、字幕播放与播放进度记录
- 音声目录树浏览支持音频、图片与视频文件识别
- 音乐资源管理、批量导入、歌词匹配、专辑封面获取与专辑筛选
- 音乐全局播放器、播放队列、顺序/循环/随机/单曲循环模式与通知中心迷你播放器
- 音乐与电影独立续播完成阈值设置，支持接近播完时自动从头播放
- 音频播放支持 MP3、WAV、FLAC、AAC、OGG、M4A、WMA、OPUS、APE 等格式
- 小说资源管理，支持 TXT、Markdown、PDF、EPUB、MOBI、AZW3 等文件格式识别与阅读
- TXT 阅读支持自动编码检测与手动编码选择，覆盖 UTF-8、UTF-16LE、简繁中文、日语、朝鲜语和西欧/中欧语言编码
- 小说阅读进度记录、PDF 首页封面生成、电子书封面/ISBN 元数据提取
- 小说 ISBN 信息插件，可通过国家图书馆 ISBN 信息来源回填作者、译者、出版社、发行年、ISBN、简介和标签
- 网站资源管理，支持 URL 录入、站点信息获取、favicon 缓存、网页截图封面与下载链接识别
- 大图预览增加安全保护，超大图片/APNG 使用受控预览，避免解码时拖垮前端
- 图片模糊模式支持按封面层作用，首页卡片和封面墙可按场景退化为纯色卡片
...
项目使用 `ffmpeg-static` 和 `ffprobe-static` 提供本地媒体探测、截图与转码能力。其中 `ffmpeg-static` 随包分发的 FFmpeg 二进制为 GPL v3 / GPL-3.0-or-later 授权；相关第三方组件声明见 [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md)。
